...
const dfs = (n, POINT) => {
    for(let i = 0; i < 5; i++){
        if(n + elixirCoin[i] <= POINT){
            user.level = parseInt(arr[n][0]);
            user.exp = arr[n][0] - user.level;
            elixir(i + 1);
            if(user.level + user.exp > arr[n + elixirCoin[i]][0]){
                arr[n + elixirCoin[i]][0] = user.level + user.exp;
                if(arr[res][0] < arr[n + elixirCoin[i]][0]) res = n + elixirCoin[i];
                arr[n + elixirCoin[i]][1] = n;
                dfs(n + elixirCoin[i], POINT);
            }
        }
        else break;
    }
...
const main = (lv, xp, po) => {
    user.level = lv; user.exp = xp;//Here will get user level, exp and point.
    const POINT = po;
    ...
    arr[0][0] = user.level + (user.exp / 100);
    res = 0;
